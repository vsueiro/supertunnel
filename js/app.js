let app = {
  ...
  data: {
    ...
    load: () => {
      ...
      if (app.mobile()) file = "countries-lo";
  ...
  three: {
    create: {
      ...
      tunnel: () => {
        ...
        app.three.tunnel.add(app.three.cylinder, app.three.chord);
      },
      ...
      graticule: () => {
        ...
        let radius = app.data.earth.radius.crust * 0.99;
        ...
        let createArc = (radius, segments, full) => {
          ...
          if (full) geometry.vertices.push(geometry.vertices[0].clone());
        ...
        for (let i = 1; i < heightSegments; i++) {
          ...
          if (i % 2 == 0) {
            ...
            let arcHeightGeometry = createArc(
              heightRadius,
              widthSegments,
              true
            );
            ...
            let arcHeightLine = new THREE.Line(
              ...
              // Increses contrast of main lines
              i == heightSegments * 0.5 ? strong : subtle
            );
      ...
      countries: (json) => {
        ...
        app.three.create.map3DGeometry.prototype = Object.create(
        ...
        for (let name in countries) {
          ...
          for (let face of faces) geometry.faces.push(face);
          ...
          app.three.land.add(
            (countries[name].mesh = new THREE.Mesh(geometry, materials))
          );
      ...
      stars: () => {
        ...
        let getCoordinates = function () {
          ...
          if (
            insideCircle(coordinates, outerRadius) &&
            !insideCircle(coordinates, innerRadius)
          ) {
            return coordinates;
          } else {
        ...
        for (let i = 0; i < amount; i++) {
          ...
          matrix.makeTranslation(coordinates.x, coordinates.y, coordinates.z);
      ...
      camera: () => {
        ...
        app.three.camera = new THREE.PerspectiveCamera(
          ...
          1,
          0.1,
          app.data.earth.radius.crust * 30
        );
      ...
      scene: () => {
        ...
        app.three.scene.add(app.three.universe);
      },
    ...
    update: {
      dimensions: () => {
        ...
        if (
          c.width !== c.clientWidth * px ||
          c.height !== c.clientHeight * px
        ) {
          // Resizes 3D canvas
          ...
          // Resizes 2D canvas
          app.three.renderer2D.setSize(c.clientWidth, c.clientHeight, false);
        }
      },
      ...
      camera: (reset = false) => {
        ...
        diagonal = (fov, aspect) => {
          ...
          return THREE.MathUtils.radToDeg(
            2 *
              Math.atan(Math.tan(THREE.MathUtils.degToRad(fov) * 0.5) / length)
          );
        };
      ...
      tunnel: () => {
        ...
        if (app.element.dataset.type == "air") app.three.tunnel.visible = false;
        else app.three.tunnel.visible = true;

        // Enables first-person view
        if (app.element.dataset.mode == "first-person") {
          // Rotates Earth to always match real-world North
          app.three.earth.rotation.y = THREE.Math.degToRad(
            app.data.orientation.alpha * -1
          );

          // Rotates stars so they behave just like Earth
          app.three.stars.rotation.y = THREE.Math.degToRad(
            app.data.orientation.alpha * -1
          );

          // Rotates compass needle
          app.elements.compass.style.transform =
            "rotate(" + app.data.orientation.alpha + "deg)";
          ...
          if (app.element.dataset.record == "true") app.orientation.record();
        }
      ...
      coordinates: () => {
        ...
        if (app.element.dataset.mode == "first-person") {
          ...
          app.three.sphere.rotation.x = THREE.Math.degToRad(
            app.data.user.latitude
          );
          app.three.sphere.rotation.z = THREE.Math.degToRad(
            app.data.user.longitude
          );

          // Rotates graticule so the user location coincides with the tunnel beginning
          app.three.graticule.rotation.x = THREE.Math.degToRad(
            -90 + app.data.user.latitude
          );
          app.three.graticule.rotation.y = THREE.Math.degToRad(
            app.data.user.longitude * -1
          );

          // Rotates countries so the user location coincides with the tunnel beginning
          app.three.land.rotation.x = THREE.Math.degToRad(
            -90 + app.data.user.latitude
          );
          app.three.land.rotation.y = THREE.Math.degToRad(
            180 + app.data.user.longitude * -1
          );
        ...
        // Enables third-person view
          ...
          app.three.tunnel.rotation.x = THREE.Math.degToRad(
            app.data.user.latitude * -1
          );
      ...
      collision: () => {
        ...
        let intersections = app.three.raycaster.intersectObject(
          app.three.sphere
        );
        ...
        if (intersections.length > 0) {
          ...
          let reduction =
            intersection.distance / (app.data.earth.radius.crust * 2);
        ...
        if (app.element.dataset.distance == "") type = "air";
        else if (app.element.dataset.destination !== "") type = "land";
        else type = "water";
    ...
    render: () => {
      ...
      // Renders 2D elements
      app.three.renderer2D.render(app.three.scene, app.three.camera);
      ...
      if (DeviceMotionEvent) {
        ...
        if (typeof DeviceMotionEvent.requestPermission === "function") {
          DeviceMotionEvent.requestPermission().then((response) => {
            if (response == "granted") app.orientation.initialize();
            else app.orientation.error("not granted");
          });
        }
  ...
  search: {
    ...
    request: (address) => {
      if (address) {
        ...
        app.search.parameters = new URLSearchParams(
          app.search.query
        ).toString();
    ...
    submit: (form) => {
      ...
      app.elements.address.forEach((address) => (address.value = input.value));
    },
  ...
  drag: {
    ...
    tolerance: 0.05, // 5% tolerance until handle snaps to the center of the chart
    ...
    move: () => {
      if (app.drag.grabbing) {
        ...
        app.drag.position.offset.x =
          app.drag.position.current.x - app.drag.position.initial.x;
        app.drag.position.offset.y =
          app.drag.position.current.y - app.drag.position.initial.y;

        // Calculates left and top distances (in pixels) related to parent
        app.drag.position.current.left =
          app.drag.position.initial.left + app.drag.position.offset.x;
        app.drag.position.current.top =
          app.drag.position.initial.top + app.drag.position.offset.y;
        ...
        if (
          app.drag.position.current.left >
            app.elements.area.offsetWidth * (0.5 - app.drag.tolerance) &&
          app.drag.position.current.left <
            app.elements.area.offsetWidth * (0.5 + app.drag.tolerance) &&
          app.drag.position.current.top >
            app.elements.area.offsetHeight * (0.5 - app.drag.tolerance) &&
          app.drag.position.current.top <
            app.elements.area.offsetHeight * (0.5 + app.drag.tolerance)
        ) {
          app.drag.position.current.left = app.elements.area.offsetWidth * 0.5;
          app.drag.position.current.top = app.elements.area.offsetHeight * 0.5;
        }

        // Calculates left and top distances (in percentage) related to parent
        app.drag.position.current.percentage.left =
          (100 * app.drag.position.current.left) /
          app.elements.area.offsetWidth;
        app.drag.position.current.percentage.top =
          (100 * app.drag.position.current.top) /
          app.elements.area.offsetHeight;

        // Sets top and left distances (in percentage)
        app.elements.handle.style.left =
          app.drag.position.current.percentage.left + "%";
        app.elements.handle.style.top =
          app.drag.position.current.percentage.top + "%";

        // Calculates values as if each axis was a divergent range input (from -N to +N)
        app.drag.value.x =
          ((app.drag.position.current.percentage.left - 50) / 50) *
          app.drag.range;
        app.drag.value.y =
          ((app.drag.position.current.percentage.top - 50) / 50) *
          -app.drag.range;
  ...
  labels: {
    ...
    update: {
      coordinates: () => {
        ...
        if (lat >= 0) label += Math.round(lat) + "°N, ";
        else label += Math.round(lat * -1) + "°S, ";

        if (lon >= 0) label += Math.round(lon) + "°E";
        else label += Math.round(lon * -1) + "°W";
      ...
      direction: () => {
        ...
        else label += Math.round(app.drag.value.y * -1) + "°S, ";

        if (app.drag.value.x >= 0) label += Math.round(app.drag.value.x) + "°E";
        else label += Math.round(app.drag.value.x * -1) + "°W";
      ...
      distance: () => {
        ...
        app.labels.distance.forEach((distance) => {
          // Updates label if value is different
          if (value !== distance.textContent) distance.textContent = value;
        });
      },
    },
  ...
  steps: {
    element: (number) => {
      if (number === undefined) number = app.steps.current();
    ...
    set: (number, delay = 0) => {
      setTimeout(() => {
        ...
        if (number === 3) app.steps.next(2400);
      }, delay);
    },
  ...
  events: {
    ...
    motion: () => {
      ...
      app.elements.trackButton.addEventListener(
        "click",
        app.orientation.request,
        true
      );
    },
